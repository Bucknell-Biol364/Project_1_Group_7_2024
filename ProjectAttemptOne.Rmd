...
The Hypotheses that we are testing are:
    ...
    -Is Bill Length different between penguins from the 3 different  
    islands.
...
Now we will ask you to test the second hypothesis, using the dataset fullframe, to compare the bill lengths among penguins from different islands. There is space for you to practice completing a boxplot, histogram, and barchart. If you are to get stuck, the answers are bellow for each visualization. Also attemtpt to analyze the graphs.
...
```{r}
#Without Jitter
boxplot(bill_length_mm ~ island, data = fullframe,
        xlab = "Island",
        ylab = "Bill Length",
        main = "Boxplot of Bill Length by Island",
        col = c("lightblue", "lightpink", "lightgreen"))

#With Jitter
ggplot(fullframe) +
  aes(x = factor(island), y = bill_length_mm) +
  geom_boxplot() +
  geom_jitter(color="black", size=0.4, alpha=0.9) +
  theme_cowplot() +
  ylab("Bill Length (mm)")+
  xlab("Island")
```
Analysis: Our boxplot suggests that penguins from Biscoe and Dream have a higher average Bill length then penguins from Torgersen Island.
...
#Bill Length Seperated by Island

ggplot(fullframe) +
  aes(x = bill_length_mm, color = island, fill = island) +
  geom_histogram(binwidth = 0.5, position = "dodge") +
  theme_cowplot()+
    xlab("Bill Length") + 
  ylab("Count")
...
ggplot(fullframe) +
  aes(x = bill_length_mm,  fill = island) + 
  geom_density(alpha=.3) +
  theme_cowplot()+
  xlab("Bill Length (mm)") + 
  ylab("Count")
...
```
Analysis: Our histogram suggests that penguins from Biscoe and Dream have a higher average Bill length then penguins from Torgersen Island. This is consistent with the boxplot.

###Answers Bar Chart
```{r}
ggplot(fullframe, aes(x = island, y=bill_length_mm), fill = Type) + 
  geom_bar(stat = "summary", position = "dodge")+ 
  scale_fill_grey()+ 
  ...
  theme_cowplot()
```
Analysis:Our Bar Chart suggests that penguins from Biscoe and Dream have a higher average Bill length then penguins from Torgersen Island. This is consistent with the boxplots and the histograms.
...
```{r}
billLaov <- aov(bill_length_mm ~ island, data = fullframe)
summary(billLaov)
```
...
```{r}
anova3 <- aov(bill_length_mm ~ sex * island, data = fullframe)
summary(anova3)
```
