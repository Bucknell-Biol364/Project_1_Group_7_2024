...
In order to run some of the functions later on, we will need to remove the 'NA's from the data set. We first should look at how many there are...
```{r}
...
...then take them out. NA values can disrupt the data, and for this particular data set, we won't need them. 
...
```{r}
lmlogspecieslength <- lm(log10bill_length ~ species, data = fullframe)
...
```{r}
lmlogsexlength <- lm(log10(bill_length_mm) ~ sex, data = torgersenpen)
