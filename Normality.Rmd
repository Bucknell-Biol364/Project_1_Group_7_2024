...
```{r}
fullframe <- drop_na(fullframe, bill_length_mm)
...
```{r}
fullframe=as.numeric(fullframe$bill_length_mm)
```
...
```{r}
lmmasshatch <- lm(Mass.final ~ Hatch, data = RxP)
summary(lmmasshatch)

lmmasspred <- lm(Mass.final ~ Pred, data = RxP)
summary(lmmasspred)

lmmassres <- lm(Mass.final ~ Res, data = RxP)
summary(lmmassres)
...
### 5. Figure out how to normalize the data

### 6. Plot residuals of normalized
